...
#%% Test(Accuracy, Adversairal Attack)"
        ...
        correct,total,test_acc, test_loss = test.accuracy(testloader, model)  # vit: Trained model
        print('test loss: %.3f, test acc %.2f %%, %.1f, %.1f' % (test_loss, test_acc, correct,total))
        
        correct,total,test_acc, test_loss = test.accuracy(attackloader, model)  # vit: Trained model
        print('test loss: %.3f, test acc %.2f %%, %.1f, %.1f' % (test_loss, test_acc, correct,total))
        ...
        accuracies = []
        examples = []
        epsilons = [0, 0.1/255, 0.3/255, 1/255, 4/255]        #epsilons = [.05, .1,.3]
        ...
        model.load_state_dict(torch.load('./model.pth')) 
        for eps in epsilons:
            acc, ex = attacks.attack(model, device, attackloader, eps) # "attackloader" : dataset
...
#test loss: 0.501, test acc 84.64 %  500
