...
def show_cams(img, mask_dic,epsilon, model_name):
    for name, mask in mask_dic.items():
        show_cam_on_image(img, mask, name, epsilon, model_name)
    
def show_cam_on_image(img, mask, name, epsilon, model_name):
    heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)
    heatmap = np.float32(heatmap) / 255
    cam = heatmap + np.float32(img)
    cam = cam / np.max(cam)
    cv2.imwrite("cam{}_{}_{}.jpg".format(name, epsilon, model_name), np.uint8(255 * cam))
