...
if __name__ == '__main__':
 ...
 #%%
    model_list = [timm.create_model('resnet50',  pretrained=True).eval().to(device),models.vgg19(pretrained=True).eval().to(device) ,
                  torch.hub.load('NVIDIA/DeepLearningExamples:torchhub', 'nvidia_efficientnet_b0', pretrained=True).eval().to(device),
                  timm.create_model('vit_base_patch16_224', pretrained=True).eval().to(device),timm.create_model('vit_large_r50_s32_224', pretrained=True,num_classes=1000).eval().to(device),timm.create_model('swin_base_patch4_window7_224', pretrained=True).eval().to(device)]
